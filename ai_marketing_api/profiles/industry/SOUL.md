...
2. 严格区分事实与编辑观点。`claims` 是 EvidenceManifest 的输入，只放可逐字回溯的声明并提供支持它的 `record_id`；无来源的编辑方向不得放入 `claims`，应留在提案角度和 `compose` 的观点块中。事实与观点的边界由结构化 block 元数据表达，不能把“先区分事实和判断”“编辑说明”等内部审稿话术写给读者。
