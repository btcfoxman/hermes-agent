...
_INDUSTRY_CONTRAST_CLICHE_RE = re.compile(
    ...
    r"后续看点|"
    r"最该(?:看|关注|盯)|"
    r"这类(?:处罚|事件|案例|治理).{0,16}(?:落到业务|落到经营(?:端|上)?|影响|变化)|"
    r"真正(?:改变|要变)的?是|"
    ...
    r"(?:结果|结论|信号|落点).{0,6}(?:很|已经|十分)?(?:清楚|明确)|"
    r"(?:最|更)?值得.{0,10}(?:盯住|关注|观察|注意).{0,8}(?:一点|的是?)|"
    r"一个很具体的变化|"
    r"(?:经营者|酒店|商家).{0,12}最怕"
    r")"
)
...
_INDUSTRY_FUNDS_REMEDY_EVENT_RESTATEMENT_RE = re.compile(
    ...
    r"(?:占用|扣留).{0,20}被要求退还|"
    r"被强制扣除.{0,20}(?:订单储备金|储备金|经营资金).{0,20}(?:退回|返还)|"
    r"(?:订单储备金|储备金|经营资金).{0,16}(?:要|将|必须|被要求).{0,8}(?:退回|返还)"
)
_INDUSTRY_FUNDS_REMEDY_SPECULATION_RE = re.compile(
    r"(?:现金|周转|经营).{0,16}(?:更充足|明显提升|空间更大|经营缓冲)|"
    r"(?:合规成本|谈判阻力|门槛).{0,16}(?:上升|更高)|"
    r"(?:议价|谈判|谈条件).{0,16}(?:被压低|更弱|空间更小|更有底气)|"
    r"(?:更硬的?(?:依据|筹码)|(?:多出|多了).{0,10}(?:依据|筹码))|"
    r"占用边界.{0,16}(?:压回|收紧|变窄)|"
    r"失去.{0,16}占用.{0,10}空间"
)
    ...
    *,
    ...
    def safe_editorial(
        ...
        *,
        ...
        # already enforced above; authored hooks and judgments require only a
        # substantive reader-facing line. CTA intent remains explicit.
            ...
            **publication_brief,
            ...
            "role_thesis": {
                ...
                "personal_ip": (
                    ...
                    "Around it, write first-person present-tense judgment about the "
                    "tradeoff and a practical principle the reader can use. Phrases "
            ...
            "authored_block_rule": {
                ...
                "personal_ip": (
                    ...
                    "result, credential, and past experience. First-person present-tense "
                    "judgment is allowed in authored transition/opinion blocks, but it "
                    "must not introduce any new biographical or business assertion. Let "
                    "the canonical personal-card block carry every actual experience."
                ),
            }[role.value],
        },
